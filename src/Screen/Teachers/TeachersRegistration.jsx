...
export default function TeachersRegistration () {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        // Create a query to check if the email already exists in the Firestore collection
        const q = query(collection(db, 'teachers'), where('email', '==', email));
        const querySnapshot = await getDocs(q);
    
        if (!querySnapshot.empty) {
          // If the query returns documents, it means the email already exists
          alert('Email already exists. Please choose a different email.');
        } else {
          // If the email does not exist, proceed to add new student data
